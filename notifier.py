...
def _build_digest_html(stories, period_label):
    """Build an inline-CSS HTML digest email body from a list of story dicts."""
    stories_html_parts = []
    for i, story in enumerate(stories):
        ...
        if source_urls:
            bullets = "".join(
                f'<li style="margin-bottom:4px;list-style-type:disc;"><a href="{_esc(u)}" style="color:#4a90d9;word-break:break-all;">{_esc(u)}</a></li>'
                for u in source_urls
            )
            src_html = (
                f'<h4 style="color:#1a1a2e;border-bottom:1px solid #e0e0e0;padding-bottom:4px;margin:10px 0 6px;">Sources</h4>'
                f'<ul style="font-size:12px;line-height:1.6;padding-left:20px;margin:0 0 8px;list-style-type:disc;">{bullets}</ul>'
            )

        stories_html_parts.append(f"""
  <div style="border:1px solid #e0e0e0;border-radius:6px;padding:18px 20px;margin-bottom:20px;background:#fafafa;">
    ...
    <ul style="line-height:1.6;padding-left:20px;margin:0 0 8px;list-style-type:disc;">{mit_html if mit_html else '<li style="list-style-type:disc;">No mitigations listed.</li>'}</ul>
    {src_html}
  </div>""")
...
<head><meta charset="utf-8"></head>
<body style="font-family:Arial,Helvetica,sans-serif;color:#222;background:#f5f5f5;padding:20px;">
<div style="max-width:680px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #ddd;">
  <div style="background:#1a1a2e;color:#fff;padding:16px 24px;">
    <h1 style="margin:0;font-size:18px;">Threat Loom Daily Digest</h1>
    <p style="margin:6px 0 0;font-size:13px;opacity:0.8;">{_esc(period_label)} &mdash; {len(stories)} stor{"y" if len(stories) == 1 else "ies"}</p>
  </div>
  <div style="padding:24px;">
    {stories_html}
    <hr style="border:none;border-top:1px solid #eee;margin:24px 0;">
    <p style="font-size:12px;color:#999;">This digest was generated by Threat Loom.</p>
    <p style="font-size:11px;color:#b0b0b0;line-height:1.5;margin-top:8px;">Disclaimer: This summary was generated by Threat Loom using large language models (LLMs). LLMs can make mistakes. Always verify important information against the original source.</p>
  </div>
</div>
</body>
...
def send_digest_email(stories, period_label):
    """Send a digest email containing clustered stories.

    Args:
        stories: List of story dicts (from synthesize_digest_story).
        period_label: Human-readable date range string for the subject line.

    Returns:
        Tuple of (success_bool, error_msg_or_None).
    """
    smtp_cfg = _get_smtp_config()
    if smtp_cfg is None:
        return False, "SMTP not configured or email notifications disabled"

    html = _build_digest_html(stories, period_label)
    subject = f"ThreatLoom Daily Digest \u2013 {period_label}"
    ok, err = _send_email(smtp_cfg, subject, html)
    if ok:
        logger.info(f"Digest email sent: {len(stories)} stories, period={period_label}")
    else:
        logger.error(f"Digest email failed: {err}")
    return ok, err
