...
def _fetch_single_feed(feed_cfg, lookback_days, since_last_fetch):
    """Fetch and process a single RSS/Atom feed.

    Returns:
        int: Number of newly inserted articles from this feed.
    """
    name = feed_cfg["name"]
    url = feed_cfg["url"]
...
def fetch_all_feeds(lookback_days=1, since_last_fetch=False):
    """Download and process all enabled RSS/Atom feeds in parallel.

    Uses a thread pool to fetch multiple feeds concurrently, bounded by
    ``MAX_FEED_WORKERS``.
    ...
    with ThreadPoolExecutor(max_workers=MAX_FEED_WORKERS) as executor:
        futures = {
            executor.submit(_fetch_single_feed, feed_cfg, lookback_days, since_last_fetch): feed_cfg["name"]
