...
- **Trend Forecasting** — Category-level trend + 3-6 month forecast. Drill into specific threat actors, malware families, and offensive tooling. Handles large article sets via iterative analysis: articles are processed in ~10k-char chunks, with later chunks refined against earlier analysis to avoid data loss. Cost estimate shown before generation with caching benefits factored in; actual cost shown after with full token breakdown (input, cache write, cache read, output).
